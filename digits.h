...
u16 toInt(const Factor& f)
{
    return 100 * f[0] + 10 * f[1] + f[2];
};

u16 multiply(const Factor& f1, const Factor& f2)
{
    return toInt(f1) * toInt(f2);
};

Factor toFactor(u16 n)
{   
    Factor f{0,0,0};
    ...
    while (n > 0)
    {
        ...
        n = static_cast<u16>(trunc(n / 10));
    }
    return f; 
};

Number toNumber(u32 n)
{
